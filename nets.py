...
class Net(nn.Module):
    def __init__(self):
        ...
        self.u12 = monai.networks.nets.BasicUNet(spatial_dims=3, in_channels=4, out_channels=3, features=(32, 32, 64, 128, 256, 32), act=('LeakyReLU', {'negative_slope': 0.1, 'inplace': True}), norm=('instance', {'affine': True}), bias=True, dropout=0.0, upsample='deconv', dimensions=None)
        self.u23 = monai.networks.nets.BasicUNet(spatial_dims=3, in_channels=7, out_channels=3, features=(32, 32, 64, 128, 256, 32), act=('LeakyReLU', {'negative_slope': 0.1, 'inplace': True}), norm=('instance', {'affine': True}), bias=True, dropout=0.0, upsample='deconv', dimensions=None)
        self.u34 = monai.networks.nets.BasicUNet(spatial_dims=3, in_channels=10, out_channels=3, features=(32, 32, 64, 128, 256, 32), act=('LeakyReLU', {'negative_slope': 0.1, 'inplace': True}), norm=('instance', {'affine': True}), bias=True, dropout=0.0, upsample='deconv', dimensions=None)
        self.u45 = monai.networks.nets.BasicUNet(spatial_dims=3, in_channels=13, out_channels=3, features=(32, 32, 64, 128, 256, 32), act=('LeakyReLU', {'negative_slope': 0.1, 'inplace': True}), norm=('instance', {'affine': True}), bias=True, dropout=0.0, upsample='deconv', dimensions=None)
    ...
    def forward(self, t0):
        ...
        ddf02 = self.u12(torch.cat((t0,ddf01), dim=1))
        # t2 = self.warp_layer(t0, ddf02)
        ddf03 = self.u23(torch.cat((t0,ddf01,ddf02), dim=1))
        # t3 = self.warp_layer(t0, ddf03)
        ddf04 = self.u34(torch.cat((t0,ddf01,ddf02,ddf03), dim=1))
        # t4 = self.warp_layer(t0, ddf04)
        ddf05 = self.u45(torch.cat((t0,ddf01,ddf02,ddf03,ddf04), dim=1))
        ...
        return [ddf01,ddf02,ddf03,ddf04,ddf05]#s,t1,t2,t3,t4,t5]#,t6,t7,t8,t9,ft0
