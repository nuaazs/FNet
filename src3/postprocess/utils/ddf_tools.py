...
def resample_3d_ddf(ddf, img):
    ...
    for i in range(3):
        ...
        fake_ddf_image_512 = ants.resample_image(
            fake_ddf_image_256, dist_shape, True, 4
        )
...
def warp_img_npy(fixed_filepath, moving_filepath, ddf_npy_filepath, real_ddf_filepath):
    ...
    moved_img = ants.apply_transforms(
        fixed=fixed, moving=moving, transformlist=[savepath]
    )
    result = {"fixed": fixed, "moving": moving, "ddf": ddf_img, "moved": moved_img}
    return result
...
def warp_img_nii(fixed_filepath, moving_filepath, ddf_nii_filepath):
    ...
    if ddf_img.shape != moving.shape:
        ...
        ants.set_spacing(
            ddf_img_reshaped, (moving.spacing[0], moving.spacing[1], moving.spacing[2])
        )
        ants.image_write(ddf_img_reshaped, "./temp_real_ddf.nii")
        moved_img = ants.apply_transforms(
            fixed=fixed, moving=moving, transformlist=["./temp_real_ddf.nii"]
        )
    else:
        moved_img = ants.apply_transforms(
            fixed=fixed, moving=moving, transformlist=[ddf_nii_filepath]
        )
    result = {"fixed": fixed, "moving": moving, "ddf": ddf_img, "moved": moved_img}
    return result
