...
def norm_list(data,input_volume):
    ...
    result = (np.array(data)-np.array(data).min())*100/(np.array(data).max()-np.array(data).min())
    result[0] = 100
    return result
...
if __name__ == '__main__':
    ...
    for index in range(len(real_ddfs)):
        ...
        input_image = ants.image_read(os.path.join(real_nii_path,f"{pname}_t0_resampled.nii"))
        ...
        for i in range(9):
            ...
            real_image_nii = ants.image_read(os.path.join(real_nii_path,f"{pname}_t{i}_resampled.nii"))
