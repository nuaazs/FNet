...
def get_tumor_location(nrrd_file_path,real_ddf_file_path,fake_ddf_file_path):
    ...
    tumor_warped_real = tumor_img.new_image_like(warp_layer(tumor_torch, torch.tensor(real_ddf_array).cuda()).cpu().detach().numpy()[0,0])
    tumor_warped_fake = tumor_img.new_image_like(warp_layer(tumor_torch, torch.tensor(fake_ddf_array).cuda()).cpu().detach().numpy()[0,0])
